...
namespace GainFrameWork.Communication.WebSockets
{
    public class WebSocketService : WebSocketBase, IService
    {
        ...
        public WebSocketService(Stream stream, TcpClient tcpClient, string header, bool noDelay, ILog logger)
            : base(logger)
        {
            _stream = stream;
            _header = header;
        ...
        public virtual void Dispose()
        {
            ...
            if (_stream.CanWrite && !_isDisposed)
            {
                ...
                _isDisposed = true;
                _logger.Info( "Sent web socket close message to client");
        ...
        protected override void OnConnectionClose(byte[] payload)
        {
            ...
            _logger.Info("Sent response close message to client");
            _isDisposed = true;
