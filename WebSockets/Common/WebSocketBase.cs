...
namespace GainFrameWork.Communication.WebSockets.Common
{
    public abstract class WebSocketBase
    {
        ...
        protected WebSocketFrameWriter _writer;
        ...
        public WebSocketBase(ILog logger)
        {
            _logger = logger;
            if (_logger == null) _logger = LogManager.GetLogger(typeof(WebSocketBase));
            _sendLocker = new object();
            _isOpen = false;
        }

        protected virtual void OpenBlocking(Stream stream, Socket socket)
        {
            _socket = socket;
            _stream = stream;
        ...
        public virtual void Send(byte[] toSend)
        {
        ...
        protected virtual void OnConnectionClose(byte[] payload)
        {
            ...
            if (args.Reason == null)
            {
                _logger.InfoFormat("Received web socket close message: {0}", args.Code);
            }
            else
            {
                _logger.InfoFormat("Received web socket close message: Code '{0}' Reason '{1}'", args.Code, args.Reason);
            }
        ...
        protected ConnectionCloseEventArgs GetConnectionCloseEventArgsFromPayload(byte[] payload)
        {
            if (payload.Length >= 2)
            {
                using (MemoryStream stream = new MemoryStream(payload))
                {
                    ...
                    {
                        _logger.WarnFormat( "Close code {0} not recognised", code);
