...
{
    public class WebSocketDaemon:IDaemon,IDisposable  
    {
        ...
        #region Ctor
        public WebSocketDaemon(IChannelParameters channelparameters,  X509Certificate2 sslCertificate, string serviceName="/gain"):this(channelparameters, serviceName)
        {
            _sslCertificate = sslCertificate;
           
        }

        public WebSocketDaemon(IChannelParameters channelparameters, string serviceName = "/gain")
        {
            _channelParameters = channelparameters;
            if (_channelParameters.TimeOut <= 0) _channelParameters.TimeOut = 2000;

            _service = serviceName;
            _logger = LogManager.GetLogger(typeof(WebSocketDaemon));
            string webRoot = "";
        ...
        private void HandleAsyncConnection(IAsyncResult res)
        {
            ...
            try
            {
                ...
                try
                {
                    tcpClient = _mainSocket.EndAcceptTcpClient(res);
                }
                ...
                {
                    _logger.Error("Error in HandleAsyncConnection", ioex);
                }
                ...
                {
                    _logger.WarnFormat("Tcp client not available or connected");
                    return;
                }
                if (_channelParameters.TimeOut > 0)
                {
                    tcpClient.Client.ReceiveTimeout = _channelParameters.TimeOut;
                    tcpClient.Client.SendTimeout = _channelParameters.TimeOut;
                }
        ...
        private Stream GetStream(TcpClient tcpClient)
        {
            ...
            catch (AuthenticationException e)
            {
                ...
                _logger.Error("Getstream error ", e);
                throw;
            }
            catch (Exception ex)
            {
                _logger.Error("Getstream error ", ex);
                throw;
            }
        }
        ...
        private  ConnectionDetails GetConnectionDetails(Stream stream, TcpClient tcpClient)
        {
            try
            {
                ...
                string header = HttpHelper.ReadHttpHeader(stream, _logger);
        ...
        public string RemoteHost(int connectionId)
        {
            lock (m_workerSocketList)
            {
                if (m_workerSocketList.ContainsKey(connectionId))
                {
                    ...
                    if (socketHandler !=null) return socketHandler.RemoteHost;
                }
            }
            return "";
        }
        ...
        public void Disconnect(int connectionId)
        {
            lock (m_workerSocketList)
            {
                ...
                {
                    var socketHandler = m_workerSocketList[connectionId];
                    if (socketHandler != null) socketHandler.Dispose();
                }
            }
        }
